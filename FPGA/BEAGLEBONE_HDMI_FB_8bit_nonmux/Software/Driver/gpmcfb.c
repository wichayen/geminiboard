...
static struct gpmc_timings fpga_timings = {
    ...
    .wr_access  = 55,   //fit time

    .rd_cycle   = 55,   //fit time

    .wr_cycle   = 55,  // fit time
					...
					u8 *txbuf, int size)
{
	...
	*/
	...
	temp = ioread8(fpga_pointer + 0x00);	// when read ther will automatically resets framebuffer address
	wmb();
